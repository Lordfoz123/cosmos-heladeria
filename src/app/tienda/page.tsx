...
function ShopFooter() {
  return (
    <footer className="w-full bg-muted/40 border-t border-border/60 py-8 mt-12 text-center">
      <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-6 px-6 max-w-7xl mx-auto">
        <div className="text-muted-foreground text-sm flex items-center gap-2">
          <svg width="18" height="18" fill="none" viewBox="0 0 20 20" className="text-primary">
            <rect x="4" y="4" width="12" height="12" stroke="currentColor" strokeWidth={2} />
            <path d="M4 4l8 7-8 5" stroke="currentColor" strokeWidth={2} />
          </svg>

          <span>&copy; 2025 Heladería Cosmos · Todos los derechos reservados</span>
        </div>

        <div className="flex gap-3 items-center">
          <a
            href="mailto:info@cosmos.com"
            className="text-primary hover:text-primary/90 hover:underline text-sm flex items-center gap-2 transition-colors"
          >
            <svg
...
export default function Page() {
  return (
    <>
      <main className="min-h-[70vh] w-full bg-background py-6 px-0">
        <TiendaOnline />
      </main>
      <ShopFooter />
    </>

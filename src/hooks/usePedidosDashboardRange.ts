...
export type Pedido = {
  ...
  fecha?: any; // Timestamp/Date/string/number
  total?: number;
  medioPago?: string; // yape | plin | izipay | tarjeta | efectivo | ...
...
export function usePedidosDashboardRange({
  ...
  fechaIni,
  fechaFin,
  pagosSoloPagados = true, // ✅ NUEVO: consistente con ventas pagadas
}: {
  pedidosCollection?: string;
  rangoIndex: number;
  fechaIni: string; // yyyy-MM-dd
  fechaFin: string; // yyyy-MM-dd
  pagosSoloPagados?: boolean;
}) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error("[usePedidosDashboardRange] onSnapshot error:", err);
        setLoading(false);
      }
    );
    return () => unsub();
  ...
  const { ini, fin } = useMemo(() => {
    if (rangoIndex < 3) {
      const [i, f] = rangosPedidos[rangoIndex].get!();
      return { ini: i, fin: f };
    }
    // ✅ evita parse raro: usa parseISO
    return { ini: startOfDay(parseISO(fechaIni)), fin: endOfDay(parseISO(fechaFin)) };
  }, [rangoIndex, fechaIni, fechaFin]);

  // Rango por FECHA DE CREACIÓN (fecha)
  ...
  const counts = useMemo(() => {
    ...
    const pagosPendientes = pedidos.filter((p) => {
      const mp = normMedioPago(p.medioPago);
      const confirmado = !!p.pagoConfirmado;

      // Pendiente = no confirmado en estos medios
      if (mp === "yape" || mp === "plin" || mp === "efectivo" || mp === "tarjeta") return !confirmado;
      return false;
    }).length;
  ...
  // ✅ Pagos por método consistente con "ventas pagadas"
  const pagosPorMetodo = useMemo(() => {
    const conteo: Record<string, number> = {};
    const monto: Record<string, number> = {};

    for (const p of pedidos) {
      if (pagosSoloPagados && !p.pagoConfirmado) continue;

      const key = normMedioPago(p.medioPago);
      conteo[key] = (conteo[key] || 0) + 1;
      monto[key] = (monto[key] || 0) + Number(p.total || 0);
    }

    return { conteo, monto };
  }, [pedidos, pagosSoloPagados]);
